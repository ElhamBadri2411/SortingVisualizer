...
const selectionSortAlgorithm = (inputArray) => {
  let len = inputArray.length;
  // the first element in each animation stored in animations is the index of the bar thats affected
  // the second element in each animation is to see if the bar is highlighted or it should be reset to original colour
  // the third element in each animation is the colour of the bar
  let animations = [];
  for (let i = 0; i < len; i++) {
